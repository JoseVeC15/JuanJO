...
SET modulos_habilitados = CASE
  ...
  ELSE ARRAY['dashboard','gastos','ingresos','proyectos','inventario','reportes','settings']::text[]
END
WHERE modulos_habilitados IS NULL OR array_length(modulos_habilitados, 1) IS NULL;

ALTER TABLE public.profiles
ALTER COLUMN modulos_habilitados SET DEFAULT ARRAY['dashboard','gastos','ingresos','proyectos','inventario','reportes','settings']::text[];
